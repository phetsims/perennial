 ...
 * @param {Function} callback - callback( metadata: {Object} )
 * @param {Function} [errorCallback] - errorCallback( {*} error, {number} statusCode )
 */
module.exports = function( options, callback, errorCallback ) {
  ...
  request( metadataURL, function( requestError, requestResponse, requestBody ) {
    if ( requestError || requestResponse.statusCode !== 200 ) {
      if ( errorCallback ) {
        errorCallback( requestError, requestResponse.statusCode );
      }
      else {
        throw new Error( requestError + ' ' + requestResponse.statusCode );
      }
    }
    else {
