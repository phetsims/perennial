...
module.exports = async function( grunt, repo, branch ) {
  ...
  const oldVersion = getRepoVersion( repo );
  assert( major === oldVersion.major, 'Generally a new release branch should have the same major version as master' );
  assert( minor === oldVersion.minor, 'Generally a new release branch should have the same minor version as master' );

  const newVersion = new SimVersion( major, minor, 0, 'phet', {
    testType: 'rc',
    testNumber: 0
  } );
